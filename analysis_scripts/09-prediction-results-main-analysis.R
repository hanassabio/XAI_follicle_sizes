## Examine predictions from the main models for total oocytes and MII oocytes.
## Requested by one of the reviewers.
...
## Predictions for total oocytes on the LOCO-CV test sets.
pred.colleggs <- fread("../input/predictions-dummy-alleggs.csv")
dim(pred.colleggs)

head(pred.colleggs, 5)

## Predictions for MII oocytes on the LOCO-CV test sets.
pred.mateggs <- fread("../input/predictions-dummy-mateggs.csv")
dim(pred.mateggs)

head(pred.mateggs, 5)

## Rename clinic names.
setnames(pred.colleggs, "Clinic", "clinic")
setnames(pred.mateggs, "Clinic", "clinic")

stopifnot(identical(sort(clinics.map$clinic), sort(unique(pred.colleggs$clinic))))
stopifnot(identical(sort(clinics.map$clinic), sort(unique(pred.mateggs$clinic))))

pred.colleggs <- pred.colleggs[clinics.map, on = "clinic"]
dim(pred.colleggs)
...
pred.colleggs <- ProcessPredictions(pred.colleggs, dt, "colleggs")
pred.mateggs <- ProcessPredictions(pred.mateggs, dt, "mateggs")

head(pred.colleggs, 2)
head(pred.mateggs, 2)
...
PlotPredictions <- function(pred, y.name, cutoff = NULL) {
  ...
  plt <- ggplot(pred, aes(x = pred, y = actual, col = clinic_id)) +
    ...
    scale_x_continuous(breaks = 0:max.pred,
                       limits = c(0, max.pred),
                       expand = expansion(add = 1)) +
    scale_y_continuous(breaks = 0:max.actual,
                       limits = c(0, max.actual),
                       expand = expansion(add = 1)) +
  ...
  ggsave(sprintf("../output/09-prediction-results-main-analysis/plot-%s-pred-vs-actual.png",
                 tolower(gsub(" +", "-", y.name))),
         plt, width = 20, height = 25, units = "cm", dpi = 700)
  plt
}

PlotPredictions(pred.colleggs, "Total Oocytes")
PlotPredictions(pred.mateggs, "MII Oocytes")

PlotPredictionsByClinic <- function(pred, y.name, cutoff = NULL) {
  stopifnot(all(c("pred", "actual", "clinic_id") %in% colnames(pred)))
  if (is.null(cutoff)) {
    max.actual <- max(pred$actual)
    max.pred <- ceiling(max(pred$pred))
  ...
  plt <- ggplot(pred, aes(x = pred, y = actual)) +
    ...
    xlab(sprintf("Predicted %s", y.name)) +
    ylab(sprintf("Actual %s", y.name)) +
    scale_x_continuous(breaks = seq(0, max.pred, 5),
                       limits = c(0, max.pred),
                       expand = expansion(add = 1)) +
    scale_y_continuous(breaks = seq(0, max.actual, 5),
                       limits = c(0, max.actual),
                       expand = expansion(add = 1)) +
    theme_bw() +
    theme(
  ...
  ggsave(sprintf("../output/09-prediction-results-main-analysis/plot-%s-pred-vs-actual-by-clinic.png",
                 tolower(gsub(" +", "-", y.name))),
         plt, width = 20, height = 20, units = "cm", dpi = 700)
  plt
}

PlotPredictionsByClinic(pred.colleggs, "Total Oocytes")
...
metrics.colleggs <- GetMetrics(pred.colleggs)
metrics.mateggs <- GetMetrics(pred.mateggs)

head(metrics.colleggs, 2)
...
PlotMetrics <- function(metrics, y.name) {
  ...
  ggsave(sprintf("../output/09-prediction-results-main-analysis/plot-perfromance-metrics-%s.png",
                 tolower(gsub(" +", "-", y.name))),
         plt, width = 20, height = 20, units = "cm", dpi = 700)
  plt
}

PlotMetrics(metrics.colleggs, "Total Oocytes")
...
PlotAbsoluteErrors <- function(pred, y.name) {
  ...
  ggsave(sprintf("../output/09-prediction-results-main-analysis/plot-absolute-error-%s.png",
                 tolower(gsub(" +", "-", y.name))),
         plt, width = 20, height = 25, units = "cm", dpi = 700)
  plt
}

PlotAbsoluteErrors(pred.colleggs, "Total Oocytes")
...
PlotErrors <- function(pred, y.name) {
  ...
  ggsave(sprintf("../output/09-prediction-results-main-analysis/plot-error-%s.png",
                 tolower(gsub(" +", "-", y.name))),
         plt, width = 20, height = 25, units = "cm", dpi = 700)
  plt
}

PlotErrors(pred.colleggs, "Total Oocytes")
PlotErrors(pred.mateggs, "MII Oocytes")

### Print Session Information  -------------------------------------------------

sink("../logs/session-info/09-prediction-results-main-analysis.Rout")
sessionInfo()
sink()
